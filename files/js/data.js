...
const originalData = {
  ...
  petList: [
    ...
    { name: 'Seahorse', fragments: 0, img: 51, farm: true, index: 34, defaultTier: 9 },
    { name: 'Moon', fragments: 10, img: 19, farm: true, index: 18, defaultTier: 10 },
    { name: 'Sarah', fragments: 0, img: 55, farm: true, index: 35, defaultTier: 11 },
    { name: 'E-77', fragments: 0, img: 48, farm: true, index: 31, defaultTier: 12 },
    { name: 'Dark Snake', fragments: 0, img: 50, farm: true, index: 33, defaultTier: 13 },
    { name: 'Windy', fragments: 0, img: 49, farm: true, index: 32, defaultTier: 14 },
    { name: 'Cerbero', fragments: 0, img: 138, farm: true, index: 49, defaultTier: 15 },
    ...
    for (let i = 0; i < oldPetList.length; i++) {
      for (let j = 0; j < data.petList.length; j++) {
        if (oldPetList.name === data.petList[j].name.replace(' ', '-')) {
          ...
          data.petList[j].farm = oldPetList.farm;
        }
      }
    }
  }
